...
import useUserAnswers from "@modules/voting/hooks/useUserAnswers";
    ...
    <div className="max-w-3xl mx-auto mt-10 space-y-10">
      <div className="mx-auto bg-[#1f2937] rounded divide-y divide-gray-700">
        <div className="flex items-center px-6 py-5 space-x-2">
          ...
          <div className="text-xl font-semibold">Your Answers</div>
        </div>
        <div className="px-4 py-5">
          ...
          {account && !data?.length && (
            <div>
              <div>Seem that you dont have any answers</div>
            </div>
          )}
          {data?.map((v) => (
            <Link
            ...
            >
              <div>
                <span className="font-semibold">{v?.poll?.title}</span>
              </div>
              <div>
                <span className="text-sm text-gray-400">You answered: </span>
                <span className="font-medium">
                  {v?.poll?.answers?.find((a) => a?.id === v?.answerId)?.name}
                </span>
              </div>
